...
select ORD_ID, ord_amount, ORD_DATE, `order`.CUS_ID, `order`.PRICING_ID, pro_name from `order`,supplier_pricing, product 
...
select c.cat_id, cat_name, pro_name, min_price from (select cat_id, p.pro_id, min(supp_price) as min_price from supplier_pricing sp, product p
